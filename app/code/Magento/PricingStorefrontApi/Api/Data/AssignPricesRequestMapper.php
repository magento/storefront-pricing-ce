 ...
 */
final class AssignPricesRequestMapper
{
    /**
     * @var string
     */
    private static $dtoClassName = AssignPricesRequestInterface::class;
    ...
    * would be built
    *
    * @param AssignPricesRequest $dto
    * @param string $key
    * @param mixed $value
    */
    private function setByKey(AssignPricesRequest $dto, string $key, $value): void
    {
        switch ($key) {
            case "prices":
                $convertedArray = [];
                foreach ($value as $element) {
                    $convertedArray[] = $this->objectManager
                        ->create(\Magento\PricingStorefrontApi\Api\Data\ProductPriceMapper::class)
                        ->setData($element)
                        ->build();
                }
                $dto->setPrices($convertedArray);
                break;
            case "price_book_id":
                $dto->setPriceBookId((string) $value);
                break;
        }
    }
}
